...
// below turns everything outside 32..126 into '?', so a game can only ever
// render printable ASCII — 95 characters, 475 bytes. Emitting just that slice
// and drawing it ourselves lets the sketch link Arduboy2Base instead of
// Arduboy2, dropping the font, drawChar and the Print vtable with it.
const FONT_FIRST = 32, FONT_LAST = 126;
const FONT_SLICE = FONT5X7.slice(FONT_FIRST * 5, (FONT_LAST + 1) * 5);
...
//#ENDIF
//#IF !AUDIO
 * Libraries needed: Arduboy2 (Arduino IDE Library Manager).
//#ENDIF
...
// Arduboy2Base, not Arduboy2: the engine renders every character itself (see
// font5x7/drawChar below), so the library's text layer is dead weight.
...
`;
  ...
  out += `// Font: Arduboy2's font5x7, printable ASCII ${FONT_FIRST}..${FONT_LAST} only.\n`;
  out += `const uint8_t PROGMEM font5x7[] = {\n${byteRows(FONT_SLICE)}\n};\n\n`;
  ...
  const packedFields = g.features.PACKED_TILES
    ? '  const uint8_t* palette;     // tile indices this scene draws with\n'
      + '  uint8_t tileShift;          // 0 = a byte per tile, else 8 >> shift bits each\n'
    : '';
  out += `struct SceneDef {\n  const uint8_t* tiles;\n${packedFields}  const ActorDef* actors;\n  const TriggerDef* triggers;\n  uint8_t w, h;               // scene size in tiles (multi-screen scenes scroll)\n  uint8_t actorCount, triggerCount;\n  uint16_t init, playerHit;\n};\n\n`;

  g.scenes.forEach((sc, i) => {
    out += `// Scene ${i}: ${sc.name}\n`;
    const packed = packTilemap(sc.tiles);
    if (packed.shift) {
      out += `// ${sc.tiles.length} tiles at ${8 >> packed.shift} bits each`
        + ` (${packed.bytes.length} + ${packed.palette.length} palette,`
        + ` was ${sc.tiles.length})\n`;
    }
    out += `const uint8_t PROGMEM scene_${i}_tiles[] = {\n${byteRows(packed.bytes)}\n};\n`;
    if (packed.palette) {
      out += `const uint8_t PROGMEM scene_${i}_palette[] = { ${packed.palette.join(', ')} };\n`;
    }
    if (sc.actors.length) {
  ...
  g.scenes.forEach((sc, i) => {
    ...
    const packed = packTilemap(sc.tiles);
    const packing = g.features.PACKED_TILES
      ? `${packed.palette ? `scene_${i}_palette` : 'nullptr'}, ${packed.shift}, `
      : '';
    out += `  { scene_${i}_tiles, ${packing}${actors}, ${triggers}, ${sc.cols}, ${sc.rows}, ${sc.actors.length}, ${sc.triggers.length}, ${scriptRef(sc.scripts.init)}, ${scriptRef(sc.scripts.playerHit)} },\n`;
  });
  out += `};\n`;
...
//#ENDIF
//#IF !BUTTON_SCRIPTS
uint8_t overriddenMask() { return 0; }
//#ENDIF

uint8_t codeAt(uint16_t pc) { return pgm_read_byte(&scriptCode[pc]); }

//#IF PACKED_TILES
// Scenes that draw few enough different tiles store several per byte behind a
// palette (see packTilemap in js/compiler.js). 8 >> shift bits per tile, packed
// low bits first, so reading one is a shift and a mask.
uint8_t unpackTile(const uint8_t* tiles, const uint8_t* palette, uint8_t shift, uint16_t i) {
  uint8_t b = pgm_read_byte(tiles + (i >> shift));
  uint8_t slot = (b >> (((uint8_t)i & ((1 << shift) - 1)) << (3 - shift))) & ((1 << (8 >> shift)) - 1);
  return pgm_read_byte(palette + slot);
}
//#ENDIF
...
uint8_t tileAt(uint8_t x, uint8_t y) {
  ...
  uint16_t i = (uint16_t)y * sceneW + x;
//#IF PACKED_TILES
  if (curScene.tileShift != 0) {
    return unpackTile(curScene.tiles, curScene.palette, curScene.tileShift, i);
  }
//#ENDIF
  return pgm_read_byte(curScene.tiles + i);
}
...
// ----------------------------------------------------------------- randomness
//
// The same xorshift32 js/emulator.js runs, down to the seed and the JS shift
// semantics (>> there is a signed shift on the value's int32 view, >>> is not),
// so wandering actors take the same path on the device as they did in the play
// test. It also keeps avr-libc's random()/random_r() out of the build, which
// they were costing about 310 bytes to provide a sequence nothing matched.

uint32_t rngState = 0xdead4a11;

uint16_t rnd(uint16_t n) {
  uint32_t x = rngState;
  x ^= x << 13;
  x ^= (uint32_t)((int32_t)x >> 17);
  x ^= x << 5;
  rngState = x;
  return n ? (uint16_t)(x % n) : 0;
}

// ---------------------------------------------------------------------- text
//
// Arduboy2::drawChar, byte for byte, without the library's text layer. The
// cell is 6 x 9: five glyph columns plus one of spacing, eight glyph rows plus
// one of line spacing, white on black. Writing straight into the frame buffer
// beats the library's per-pixel loop — a cell touches at most three buffer
// rows, so it is three masked byte writes per column instead of 54 drawPixel
// calls.
...
void drawChar(int16_t x, int16_t y, uint8_t c) {
  ...
  const uint8_t* glyph = &font5x7[(uint16_t)(c - ${FONT_FIRST}) * 5];
  int16_t row = y >> 3;          // first buffer row the cell touches
  uint8_t shift = (uint8_t)y & 7; // and how far down inside it the cell starts
  for (uint8_t col = 0; col < CHAR_W; col++) {
    int16_t px = x + col;
    if (px < 0 || px >= WIDTH) continue;
    // The spacing column and the spacing row are background, so they clear.
    uint32_t bits = (col < 5) ? pgm_read_byte(&glyph[col]) : 0;
    uint32_t mask = (uint32_t)((1UL << CHAR_H) - 1) << shift;
    uint32_t val = bits << shift;
    for (uint8_t r = 0; r < 3; r++) {
      int16_t br = row + r;
      uint8_t m = (uint8_t)(mask >> (r * 8));
      if (m == 0 || br < 0 || br >= HEIGHT / 8) continue;
      uint8_t& b = arduboy.sBuffer[br * WIDTH + px];
      b = (b & (uint8_t)~m) | ((uint8_t)(val >> (r * 8)) & m);
    }
  }
}
...
//#ENDIF
  ...
  for (uint8_t i = 0; i < actorCount; i++) {
    ...
    actors[i].frame = 0;
    actors[i].timer = rnd(ACTOR_MOVE_INTERVAL);
    actors[i].anim = rnd(ANIM_INTERVAL);
    actors[i].dir = 1;
...
int16_t evalExpression(uint16_t at, uint8_t len) {
  ...
  for (uint16_t i = at; i < end; i++) {
    ...
    if (op == ${EX.PUSH_CONST}) {          // PUSH_CONST
      ...
      stack[sp - 1] = n > 0 ? (int16_t)rnd(n) : 0;
    } else {
      int16_t b = stack[--sp];
  ...
  for (uint16_t i = from;; i++) {
    ...
    if (c == 1) {
      ...
      while (n--) {
        ...
        if (draw) drawChar(cx, cy, digits[n]);
        cx += 6;
        shown++;
      }
      continue;
...
//#ENDIF
      //#IF OP_SEED_RNG
      case 42: // SEED_RNG
        rngState ^= ((uint32_t)arduboy.frameCount * 2654435761UL) ^ micros();
...
void setup() {
//#IF MINIMAL_BOOT
  // Arduboy2::begin() without the parts a finished game does not need. The
  // logo animation alone costs about a kilobyte, most of it the bitmap and the
  // drawBitmap() nothing else in this sketch calls. Holding B at power-on
  // still reaches the sound on/off setting.
  arduboy.boot();
  arduboy.display();
